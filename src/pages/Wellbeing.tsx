...
const DEMO_DAILY: DailyCheckin[] = Array.from({ length: 28 }, (_, i) => ({
  energy_level: [
    40, 55, 50, 35, 60, 65, 58,
    45, 70, 62, 48, 55, 72, 68,
    75, 60, 80, 55, 65, 78, 82,
    70, 85, 72, 68, 90, 76, 74,
  ][i],
                  ...
                  return (
                    <div key={d.date} className="flex-1 flex flex-col items-center group relative">
                      <motion.div
                        ...
                        className="w-full rounded-t-sm min-w-[4px] max-w-[20px] mx-auto"
              ...
              <div className="flex gap-1 mt-1">
                {dailyData.map(d => (
                  <div key={d.date} className="flex-1 text-center">
                    <span className="text-[8px] text-muted-foreground">{d.weekDay}</span>
                  </div>
                ))}
              </div>
            </div>
